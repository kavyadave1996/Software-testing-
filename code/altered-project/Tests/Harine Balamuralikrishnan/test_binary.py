...
class TestBinaryResponseData:
    ...
    def test_binary_suppresses_when_terminal(self, httpbin):
        ...
        # If server error, skip assertion with explanation
        if isinstance(r, str) and "A server error occurred" in r:
            pytest.skip("httpbin server error: cannot test binary suppression")
    ...
    def test_binary_included_and_correct_when_suitable(self, httpbin):
        ...
        if isinstance(r, str):
            if "A server error occurred" in r:
                pytest.skip("httpbin server error: cannot test binary output")
            r = r.encode()
        assert r == expected
