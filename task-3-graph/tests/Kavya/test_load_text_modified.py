...
def test_load_text_file_raises_oserror(tmp_path):
    # Create a file and remove it to trigger OSError when loading
    file_path = tmp_path / "nonexistent.txt"
    arg = KeyValueArg(orig=str(file_path), value=str(file_path))
    with pytest.raises(ParseError):
        load_text_file(arg)