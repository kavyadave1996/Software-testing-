...
def get_color(
    ...
    if isinstance(color_code, dict):
        # Use .get() to safely get shade or None if missing
        return color_code.get(shade, None)
    else:
        return color_code
...
def test_get_color(color, shade):
    palette = {
    ...
    if color not in palette:
        ...
        if isinstance(color_value, dict):
            if shade in color_value:
                assert result == color_value[shade]
            else:
                assert result is None
        else:
            assert result == color_value

def test_get_color_with_missing_color():
    # Color not in palette
    assert get_color("not_a_color", "500") is None

def test_get_color_with_non_dict_color_value():
    # Use a palette with a direct string value instead of dict
    palette = {PieColor.GREEN: "#00FF00"}
    result = get_color(PieColor.GREEN, "any_shade", palette=palette)
